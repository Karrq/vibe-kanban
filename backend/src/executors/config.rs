...
lazy_static::lazy_static! {
    pub static ref AGENT_CONFIGS: HashMap<&'static str, AgentConfig> = {
        ...
        m.insert("ccr", AgentConfig {
            path_command: "claude-code-router",
            npx_package: "@musistudio/claude-code-router",
            args: "code -p --dangerously-skip-permissions --verbose --output-format=stream-json",
        });

        m.insert("sst-opencode", AgentConfig {
            path_command: "opencode",
            npx_package: "opencode-ai@latest",
            args: "run --print-logs",
        });
